...
import com.j1.health.spider.process.medecine.DrugAttributeProcess;
...
public class DrugClient {
	...
	public static void main(String[] args) throws Exception {
		...
		DrugAttributeProcess process = (DrugAttributeProcess) SpringContextUtil.getBeanbyName("drugAttributeProcess");
		//使用线程的方式执行爬虫job
		DrugService drugService = (DrugService) SpringContextUtil.getBeanbyName("drugService");
		...
		List<Map<String,Object>> list = drugService.getDrugList(pageMap);
		...
		if(null != resultList && resultList.size() > 0){
			...
			int result_code = drugService.insertDrugByBatch(resultList);
